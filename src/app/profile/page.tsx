...
import { Bell, ChevronRight, Globe, MapPin, Shield } from "lucide-react";
...
import { ProfileIdentitySection } from "@/components/user/profile-identity-section";
      ...
      <div className="flex flex-col gap-8">
        ...
        <section className="rounded-[2rem] border border-white/10 bg-slate-950/60 p-6 shadow-2xl shadow-emerald-950/10 sm:p-8">
          {loading ? (
            ...
            <ProfileIdentitySection
              ...
              kycStarting={kycStarting}
              kycError={kycError}
              fileInputRef={fileInputRef}
              onPickAvatarClick={() => fileInputRef.current?.click()}
              onAvatarFile={onAvatarPicked}
              onClearAvatar={clearAvatar}
              onNameDraftChange={setNameDraft}
              onBeginEditName={beginEditName}
              onCancelEditName={cancelEditName}
              onSaveName={() => void saveName()}
              onStartKyc={() => void startSingpassKyc()}
            />
          ) : (
        ...
        <section className="grid gap-4 lg:grid-cols-2">
          ...
          <ProfileSelectSetting
            ...
            disabled={savingMarket || loading || !profile || marketLocked}
            locked={marketLocked}
